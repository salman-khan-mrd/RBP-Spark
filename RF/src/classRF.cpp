...
void classRF(double *x, int *dimx, int *cl, int *ncl, int *cat, int *maxcat,
    ...
    int nsample0, mdim, nclass, addClass, mtry, ntest, nsample, ndsize,
            mimp, nimp, near1, nuse, noutall, nrightall, nrightimpall,
            keepInbag, nstrata;
    ...
    if (oobprox) {
        oobpair = (int *) S_alloc_alt(near1*near1, sizeof(int));
    }
    //printf("nsample=%d\n", nsample);
    ...
    for(jb = 0; jb < Ntree; jb++) {
        ...
        /*  DO PROXIMITIES */
        if (iprox) {
            computeProximity(prox, oobprox, nodex, jin, oobpair, near1);
    ...
    if (iprox) {
        for (n = 0; n < near1; ++n) {
            for (k = n + 1; k < near1; ++k) {
                prox[near1*k + n] /= oobprox ?
                    (oobpair[near1*k + n] > 0 ? oobpair[near1*k + n] : 1) :
                        Ntree;
                        prox[near1*n + k] = prox[near1*k + n];
            }
            prox[near1*n + n] = 1.0;
        }
        if (*testdat) {
